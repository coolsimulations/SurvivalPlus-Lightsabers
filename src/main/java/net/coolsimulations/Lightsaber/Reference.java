package net.coolsimulations.Lightsaber;

public class Reference {
	public static final String MOD_ID = "lightsaber";
	public static final String MOD_NAME = "SurvivalPlus Lightsabers";
	public static final String VERSION = "0.1.7b";
	public static final String ACCEPTED_VERSIONS = "[1.19.4]";
	public static final String CLIENT_PROXY_CLASS = "net.coolsimulations.Lightsaber.proxy.ClientProxy";
	public static final String SERVER_PROXY_CLASS = "net.coolsimulations.Lightsaber.proxy.ServerProxy";
	public static final String DEPENDENCIES = "required-after:survivalplus@[0.1.7b,);";
	
	public static final String LIGHTSABER_CONFIG_FILE = "SurvivalPlus/Lightsaber.conf";
	public static final String CONFIG_CATEGORY_VALUES = "lightsaber.configgui.values";
}
